...
interface IReviewWithUser extends Reviews{
  createdBy : User
}
  ...
  return (
    <Layout hasTabBar title="나의 캐럿">
      <div className="px-4">
        ...
        {data?.reviews?.map(review => <div key={review.id} className="mt-12">
          <div className="flex space-x-4 items-center">
            <div className="w-12 h-12 rounded-full bg-slate-500" />
            <div>
              <h4 className="text-sm font-bold text-gray-800">{ review.createdBy.name}</h4>
              <div className="flex items-center">
                {Array(5).fill(0).map((_, i) => <svg
                  key={i}
                  className={cls("h-5 w-5 ", review.score >= i+1 ?"text-yellow-400 ":"text-gray-400 ")}
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 20 20"
                  fill="currentColor"
                  aria-hidden="true"
